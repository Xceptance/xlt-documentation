...
* The load profile configuration of a load test has been enhanced:
    * It is now possible to define the ramp-up period and the shutdown period of a load test. See the section [Load Test Phases](/xlt/manual/load-configuration/#load-test-phases) for all the details.
    * The load profile editor accepts not only a constant numeric value for the load factor, but also a function (such as "0/0.1 15m/1.0 1h/2.5") for a load factor that varies over time. See the section [Load Factor](/xlt/manual/load-configuration/#load-factor) for more information on load factor functions.
