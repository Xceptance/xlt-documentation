---
title: Scenarios
description: "How to set up and manage monitoring scenarios in XTC.\n"
sidebar:
  order: 430
---

:::info[Role Required]
To view monitoring scenarios, your account must at least have the role of a [tester](/xtc/basics/projects/#user-roles-within-a-project) within the project.
:::

:::info[Role Required]
To create, edit and delete monitoring scenarios, your account must at least have the role of a [test manager](/xtc/basics/projects/#user-roles-within-a-project) within the project.
:::

## Setting Up Monitoring Scenarios

The basis for all monitoring scenarios is a set of [XLT test cases](/xlt/manual/test-development/) that will be run continuously. These tests are preferably organized in a test suite, which is located in the repository you defined in the monitoring [project configuration](/xtc/basics/project-configuration/#repository).
...
![Creating a new monitoring scenario.](/images/xtc/monitoring_newScenario.png)
*Creating a new monitoring scenario.*  

The new scenario will now show up in the list. It will be disabled by default and it will automatically use the [scenario default settings](/xtc/monitoring/monitoring-configuration/#scenario-defaults). You can now adjust these settings as needed by [editing the scenario](#managing-existing-scenarios).
...
![Overview of all existing scenarios in the project.](/images/xtc/monitoring_scenarios_overview.png)
...
To pause a scenario, open the context menu for the scenario, and click _Pause_. You will be prompted to enter the date/time until which you want to pause the scenario. Alternatively, you can pause a scenario directly from the monitoring project's [_Dashboard_](/xtc/monitoring/dashboard/) page. To unpause a scenario, click _Continue_ in the context menu.

## Scenario Details

You can open the detail view of a scenario by clicking the scenario name. There you can adjust the [scenario settings](#scenario-settings) and [quality sensors](#quality-sensors) as needed, and update basic [notification settings](#notifications). Otherwise the [scenario defaults](/xtc/monitoring/monitoring-configuration/#scenario-defaults) will be used.

### Scenario Settings

:::info[Role Required]
To edit scenario settings, your account must at least have the role of a [test manager](/xtc/basics/projects/#user-roles-within-a-project) within the project.
:::
...
![Switch between default and custom values by using the _Use Default_ checkbox.](/images/xtc/monitoring_scenario_useDefault.png)
...
**When** an alert is triggered and **to whom** notifications are sent is defined by the scenario's [**quality sensors**](#quality-sensors) and the [**notification lists**](/xtc/monitoring/monitoring-configuration/#notification-lists) these sensors are linked to.

### Quality Sensors

:::info[Role Required]
To change quality sensors of a scenario, your account must at least have the role of a [test manager](/xtc/basics/projects/#user-roles-within-a-project) within the project.
:::

The most crucial part of the settings is to define **success metrics** which will be validated during the scenario execution. Violations cause the scenario to be treated as failed. Success metrics are defined as [quality sensors](/xtc/monitoring/monitoring-configuration/#quality-sensors) in the monitoring project's configuration.
...
![List of quality sensors for a monitoring scenario.](/images/xtc/monitoring_scenario_qualitySensors.png)
