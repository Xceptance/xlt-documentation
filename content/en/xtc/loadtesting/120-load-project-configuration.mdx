...
description: "Special configuration settings for load test projects in XTC.\n"
sidebar:
  order: 120
---

The _Configuration_ of a load test project is very similar to the basic [project configuration](/xtc/basics/project-configuration/), but there are a few special settings for this project type:
...
In addition to the [basic repository settings](/xtc/basics/project-configuration/#repository) which can be defined for load test projects and monitoring projects alike, there are a few additional options for load test projects concerning the compiling of the test suite:
...
For full details on prerequisites, retention rules, and automated uploads via the REST API, see [Code Archives](/xtc/loadtesting/code-archives/).
...
![](/images/xtc/loadtest_selectBuildTool.png)
**
...
:::info[Role Required]
To view default sharing settings, your account must at least have the role of a [tester](/xtc/basics/projects/#user-roles-within-a-project) within the project.
:::

:::info[Role Required]
To edit default sharing settings, your account must at least have the role of a [test manager](/xtc/basics/projects/#user-roles-within-a-project) within the project.
:::

In _Sharing_, you can define a default for the share expiration time of [load test results](/xlt/advanced/results/#sharing-results) and [load test reports](/xlt/manual/reports/#sharing-a-report) for easier project management. Each result or report sharing will offer this time as a default (but individual expiration times may still be configured if needed). Later on, all shares that use these defaults can be either deactivated, extended, or reactivated at once when required.
...
:::info[Role Required]
To view properties, your account must at least have the role of a [tester](/xtc/basics/projects/#user-roles-within-a-project) within the project.
:::

:::info[Role Required]
To edit properties, your account must at least have the role of a [test manager](/xtc/basics/projects/#user-roles-within-a-project) within the project.
:::

In _Properties_, you can globally define [properties](/xlt/manual/test-suite-configuration/) or [secret properties](/xlt/manual/test-suite-configuration/#secret-properties) to use for test execution.

[Secret properties](/xlt/manual/test-suite-configuration/#secret-properties) behave the same as [regular properties](/xlt/manual/test-suite-configuration/). However, their values will be masked with ****** both in the load test report and in the result data set.
...
Properties configured at project level apply to all new load tests alike, while [properties defined at a certain load test](/xtc/loadtesting/lt-settings/) apply to that load test only. Load-test-level properties will overwrite project-level properties. Properties that are not set in XTC will be read from the project data.
...
:::note[What about minor version changes?]
