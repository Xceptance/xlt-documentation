---
title: Organizations
description: "Organizations in XTC and user roles within organizations.\n"
sidebar:
  order: 45
---
...
In the **Security** tab, you may enable or disable [user invitations](#inviting-users-to-join-xtc) or define [authorization requirements](#mandatory-login-requirements) for the members of your organization.

In the **Integrations** tab, you can integrate XTC with your Slack Workspace to be notified of certain events like load test status changes. See the [Slack Integration](/xtc/integrations/slack/) for further details.
...
* a **billing manager** has permission to view and evaluate [resource usage](/xtc/loadtesting/resource-usage/) of the organization, and
...
![Footer displaying the user's role in the organization.](/images/xtc/ui_orgRole.png)
...
To add new members to the organization, select _Members_ in the menu on the left. You will see the list of members, including their [Login Provider/2FA settings](#mandatory-login-requirements) and the time of their last login.
...
:::note
The _Members_ page of an organization shows the [login provider](/xtc/basics/user-accounts/#connecting-existing-xtc-accounts-to-an-external-login-provider) for every member and whether or not a member has [2FA enabled](/xtc/basics/user-accounts/#two-factor-authentication-2fa). This way, organization admins can easily check if certain security requirements imposed by the organization are met.
:::

Explicitly assigning organization members is usually not necessary in your daily work, as organization membership and roles are assigned implicitly by [adding and removing project members](/xtc/basics/projects/#adding-and-removing-project-members).
...
As an org admin you may invite users who do not have an XTC account yet to join your org in XTC (if this feature is [enabled](#organization-configuration)). Simply [add these users to your organization](#adding-members-to-an-organization) as usual, but make sure you activate the _Invite Users_ toggle.
...
:::info[Role Required]
To create, update and delete user groups, your account must have the role of an [organization administrator](/xtc/basics/projects/#user-roles-within-a-project) within the project.
:::
...
When a user is a member of an organization, but does not yet or no longer fulfill the current requirements imposed by the organization (mostly security-related), the user should be locked out, either [automatically by the system](#mandatory-login-requirements) or manually by an admin of the organization. When locked, the user remains a member of this organization, but can no longer see any data or perform any action.
...
Organization admins may impose mandatory login requirements on users of their organization. They may require users to be authenticated using specific [login providers](/xtc/basics/user-accounts/#connecting-existing-xtc-accounts-to-an-external-login-provider) or [2FA](/xtc/basics/user-accounts/#two-factor-authentication-2fa).
...
![Example of an admin about to lock themselves out of an organization by enabling mandatory 2FA.](/images/xtc/login_requirement_2fa_violated.png)
...
![Mandatory External Login Provider with a specific issuer URL.](/images/xtc/login_requirement_sso.png)
