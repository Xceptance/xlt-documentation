...
{{% note notitle %}}
The API does not cover all functionality. Especially user, role, and project management, and settings management are excluded intentionally at the moment.{{% /note %}}
...
Now run the curl command. XTC will return a JSON response. Look for the `access_token` field. Note that this access token expires automatically after one hour, but you can create new tokens at any time. For detailed instructions on API-driven code archive uploads, see [Code Archives]({{< relref "122-code-archives" >}}).
