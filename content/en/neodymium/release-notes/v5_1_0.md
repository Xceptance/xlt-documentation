...
description: "This Neodymium update mostly centers about improving our
  [reporting](Reports) to give the results reviewer a better overview and a
  quicker way to identify issues, bugs or needed changes in the performed
  tests.\n"
sidebar:
  label: 5.1.0
  order: 840
...
- Feature: Create [Google Lighthouse Reports](https://github.com/Xceptance/neodymium/wiki/Reports#lighthouse) directly from test automation runs
...
- Feature: [Improved Screenshots](https://github.com/Xceptance/neodymium/wiki/Advanced%20Screenshots)
...
- Feature: [Test Data Overview](https://github.com/Xceptance/neodymium/wiki/Reports#add-test-data-to-report) on each Test Case
  - To have a better overview on which data was used we now add an overview in a JSON format to each test case
- Feature: [JSON Assertions](https://github.com/Xceptance/neodymium/wiki/Reports#add-json-compare-to-report)
...
- Improvement: Enhanced [Environment Info](https://github.com/Xceptance/neodymium/wiki/Reports#add-custom-data-to-environment-section-of-the-report). Allure offers an environment overview, which needs to be handled by altering a file. To utilize this feature better, Neodymium now offers two new features
...
- Improvement: In-Page PopupBlocker
  - In cases where you face nondeterministic PopUps which affects your tests run, we added an automatic [PopUpBlocker](https://github.com/Xceptance/neodymium/wiki/Popup-Blocker).
- Feature: [Seperate Sessions for Setup and Cleanup](https://github.com/Xceptance/neodymium/wiki/Seperate-Browser-Sessions)
  - It is now possible to run the `@BeforeEach` and `@AfterEach` in separate, clean browser sessions.
- Improvement: Neodymium [properties](/neodymium/configuration/neodymium-properties/) can now be overwritten by system properties and environmental variables.
  - This allows pipeline runs to be configured independently of the state of the repository
- Improvement: [URL Validation](https://github.com/Xceptance/neodymium/wiki/Neodymium-configuration-properties#url-properties)
...
- Improvement: [Support Passing Command Line Arguments](https://github.com/Xceptance/neodymium/wiki/Multi-browser-support#browser-profile-configuration) To Driver
