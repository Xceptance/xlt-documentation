...
One machine runs your [master controller]({{< relref "xlt-basics#mcmaster-controller" >}}), which controls several machines running the [agent controllers]({{< relref "xlt-basics#acagent-controller" >}}) and [agents]({{< relref "xlt-basics#agents" >}}).
...
To use the **gce_admin** tool to create, list, and stop Google Cloud instances (which will run your [agent controllers]({{< relref "basics#acagent-controller" >}}) and [agents]({{< relref "basics#agents" >}})), navigate to `<XLT>/bin/` and run:
...
If your AWS EC2 instances need custom configuration data (_user data_, in Amazon terminology), e.g., for setting a custom agent controller password or adding content to `/etc/hosts`, you can also specify this data during `ec2_admin` startup. There are three supported ways to pass the user data:
