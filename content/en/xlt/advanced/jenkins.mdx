---
title: CI/CD
description: "Learn how to integrate XLT into CI/CD pipelines and evaluate results.\n"
sidebar:
  order: 80
---
...
Once you're done with the configuration, start your build project manually to verify that everything works as expected. If all goes well, your build should show <span style={{ color: "darkblue" }}>SUCCESS</span>. If the load test cannot be executed for any reason (wrong configuration, etc.), the build will be marked as <span style={{ color: "red" }}>FAILED</span>. If the load test runs to completion, but the configured [success criteria](#success-criteria) are violated, the build will be marked as <span style={{ color: "orange" }}>UNSTABLE</span>.
...
![Build Steps in a Job](/images/how-to/jenkins/BuildSteps.png)
