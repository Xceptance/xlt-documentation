---
title: Cloud Setup
description: "Learn how to set up a cluster of test machines in the cloud for
  load generation.\n"
sidebar:
  order: 60
---
...
![Distributed Load Generation Environment](/images/user-manual/cloud_setup.svg)
*Distributed Load Generation Environment*

One machine runs your [master controller](/xlt/manual/xlt-basics/#mcmaster-controller), which controls several machines running the [agent controllers](/xlt/manual/xlt-basics/#acagent-controller) and [agents](/xlt/manual/xlt-basics/#agents).
...
:::note[How many machines do I need?]
It can be hard to determine the number and size of load testing machines you'll need for your setup. Here's [a small guide about test sizing](/xlt/how-tos/test-sizing/).
:::
...
To use the **gce_admin** tool to create, list, and stop Google Cloud instances (which will run your [agent controllers](/xlt/manual/xlt-basics#acagent-controller) and [agents](/xlt/manual/xlt-basics#agents)), navigate to `<XLT>/bin/` and run:
...
{/*  TODO: }Is "user data" parameter still valid?  */}}
...
When starting Amazon EC2 machine instances, you can also [specify a name tag](#aws-name-tag) that will be assigned to each started instance. This name tag can be used later to filter the running instances (e.g., when listing or terminating them).
...
To terminate AWS instances using `ec2_admin` (<kbd>t</kbd>), you will be prompted to select a region and filters as [described above](#listing-running-aws-instances).
