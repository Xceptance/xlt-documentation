...
The data for the dynamic charts are contained in additional JSON files in the report directory, which increases the amount of disk space the report uses.
If you don't need the dynamic charts or want to save disk space, you can disable this feature in the `config/reportgenerator.properties` file:
...
Note that this is just a substring search, but it's much faster than the "urlPattern" variant. However, you can't capture any data besides the full URL with "{u}".
...
If it's taking too long to create the report, check to see if your merge rules could benefit from the new URL substring match feature.
...
* In the Result Browser, the request name, which is typically the last part of the request URL, now contains the query string. This change makes it easier to distinguish one request from another, especially API requests.
