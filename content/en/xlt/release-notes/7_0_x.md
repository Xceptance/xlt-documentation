...
The design of the report pages has been modernized in details.

Many details of the report The design of the report pages has been modernized in details.

You will notice the page header is now sticky and therefore always visible. If you configured a project name in your test suite, that name is shown in the header as well. When dealing with multiple projects at the same time, this always-visible information may help to quickly tell which project a report belongs to.

The report pages are also more responsive to different devices. On wider screens, the font size gets a bit larger, so reading the data tables is easier on the eye. On smaller screens, especially on mobile devices, the navigation bar turns into a hamburger menu.
...
Almost all other third-party libraries have been updated as well, including Selenium. Since the new Selenium does no longer support the "legacy mode" of `FirefoxDriver`, we had to remove that mode from XLT as well. Same for `OperaDriver`.
