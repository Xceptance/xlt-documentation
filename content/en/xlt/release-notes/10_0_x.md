...
description: >
    Virtual Threads, Private Machine Mode, Reporting Improvements, HtmlUnit 5.x
...
Since we have updated HtmlUnit to the new major version 5.x, there may be further incompatible API changes. Adjust your code accordingly.

* The method `HttpClientConverter.nameValuePairsToHttpClient(List)` does no longer exist. This method was often used in tandem with `URLEncodedUtils.format(List, String)` to format query string parameters. Now there is an alternative method `HttpUtils.toQueryFormFields(List, Charset)` that performs these two steps in one go.
