...
* **config** contains all [test settings]({{< relref "test-suite-configuration" >}}
) files. The only file that is mandatory is `log4j.properties`, but usually there will be several property files containing test settings. XLT will look for all properties in this directory by default, but the location may be overridden by the system property `com.xceptance.xlt.configDir` and, alternatively, the environment variable `XLT_CONFIG_DIR`, which both make it possible to specify the configuration directory to use (system property takes precedence; the directory location is required to be within the test suite in any case).
