...
description: "Learn how to set up the test suite to run as a performance test
  and what load profiles are supported.\n"
sidebar:
  order: 470
---

Before running a performance test, you must configure the load applied to your application. The settings for a specific load test profile are stored in a property file within the test suite. You can create multiple configuration files to prepare different scenarios in advance, switching between them by changing the `com.xceptance.xlt.testPropertiesFile` property in `project.properties`. For details on properties and syntax, see [Load Test Profile Configuration](/xlt/manual/test-suite-configuration/#load-test-profile-configuration). This section covers the basic concepts of [load models](#load-models), [load factor](#load-factor), and [load profiles](#load-profiles), and how to define your target load.
...
![](/images/user-manual/chart_rampup.svg)
**
...
![](/images/user-manual/chart_rampup2.svg)
**
...
Separate multiple pairs with spaces, commas, semicolons, or tabs. The time format supports standard [time periods](/xlt/manual/test-suite-configuration/#time_period_values).
...
![](/images/user-manual/chart_variable_load_factor.svg)
**
...
:::warning[Ramp-Up is Overwritten by Load Functions]
...
![Load Test Profile Configuration](/images/user-manual/chart_testprofile.svg)
