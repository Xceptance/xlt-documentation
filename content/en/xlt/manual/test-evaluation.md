...
{{% note notitle %}}
To learn more about the different report types XLT can create, follow up [here]({{< relref "reports" >}}). For more information on how to customize your reports, please have a look at the [Report Options]({{< relref "report-options" >}}).
{{% /note %}}
...
Unlike on the *Requests* page, the recorded data is not grouped by request name but every single recorded slow request is listed here.
...
The *Custom Timers* page includes all [timers]({{< relref "custom-data/#custom-timers" >}}) that have been placed individually within the test code. The chart and data description is identical to the *Actions* section.

### Custom Data

The *Custom Data* page contains two sections. In case [custom samplers]({{< relref "custom-data/#custom-values" >}}) have been run during the test, or if your test scenarios are instrumented to log custom values, the collected data is shown in the *Custom Values* section. If the test scripts have also been collecting [custom data logs]({{< relref "custom-data/#custom-data-logs" >}}), they will be available for download in the *Custom Data Logs* section.

### Web Vitals

If the load test was run as an [automated client performance test]({{< relref "client-performance/#measuring-client-performance-with-xlt" >}}) with Chromium browsers, this page displays the scores for all supported Web Vitals for each action in your scenarios. The score value is the 75th percentile of all observations and is rated as *good*, *needs improvement*, and *poor* using the colors green, yellow, and red according to Web Vital-specific thresholds.

### External Data

All [external data]({{< relref "custom-data#external-data" >}}) gathered by other tools during the test run is shown here according to the configuration.
