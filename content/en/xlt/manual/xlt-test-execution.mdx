...
description: "How to configure and execute tests using the Mastercontroller.\n"
sidebar:
  order: 310
---
...
This chapter explains the setup of a base test environment and how to configure the components of XLT correctly. Please ensure you have read the [Architecture](/xlt/about/architecture/) guide first. You should also have created a test suite by now (see [Test Development](/xlt/manual/test-development/)) and likely configured a first test (see [Test Setup](/xlt/manual/test-setup/)).
...
The agent controller starts and listens on the specified port. With default logging enabled (WARN), no messages are printed during the agentcontroller startup. When [info logging is enabled](/xlt/manual/environment-configuration/#agent-controller-logging), the output might look like this:
...
For more information on configuration options for agentcontrollers, see [Environment Configuration](/xlt/manual/environment-configuration/#agent-controllers).
...
Starting with [XLT 6.2.0](/xlt/release-notes/v6_2_x/#run-start-command-independently-from-upload-command), the start command can be run separately in non-interactive mode. Please ensure that you have uploaded the test suite before or otherwise the load test will finish immediately.
