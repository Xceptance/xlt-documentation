...
---
## Mastercontroller 
The `mastercontroller` is the primary tool for managing the entire load test. It calculates load distribution, begins and ends the test, and also deploys the test suite to all `agentcontroller` instances. At the end, the `mastercontroller` saves test results on the file system. It can be run using [interactive mode]({{< relref "test-execution#interactive-mode" >}}), [auto mode]({{< relref "test-execution#auto-mode" >}}), or [non-interactive mode (scripted commands)]({{< relref "test-execution#non-interactive-mode-scripted-commands" >}}).

## Usage

- `mastercontroller [<other options>]` runs in [interactive mode]({{< relref "test-execution#interactive-mode" >}}). Any command can be chosen to be executed next.
- `mastercontroller --help` displays all available options and a brief guide on their usage.
- `mastercontroller -auto [<other options>]` runs a load test in [auto mode]({{< relref "test-execution#auto-mode" >}}) by automatically executing all commands.

### Example

 This will run a load test in [auto mode]({{< relref "test-execution#auto-mode" >}}), in combination with [embedded mode]({{< relref "test-execution#embedded-mode" >}}):
#### Windows
```powershell 
cd <XLT>\bin
...
- `-embedded` utilizes a single embedded agent controller, running the test in [embedded mode]({{< relref "test-execution#embedded-mode" >}}).

- `-faf` Usage is deprecated. It is recommended to use `-auto` instead.

- `-noDownload` is for use with auto mode only. Test results are not downloaded. This option is ignored in sequential/[non-interactive mode]({{< relref "test-execution#non-interactive-mode-scripted-commands" >}}).

- `-o <dir>`  stores downloaded test results in the specified directory. 

- `--only-download <dataTypeList>` restricts download to only the given result data types in a comma-separated list. Applicable only for [downloads in non-interactive mode]({{< relref "test-execution#downloads-in-non-interactive-mode" >}}). 

- `-pf <file>` can be used to utilize a separate properties file, which overrides values in the `mastercontroller.properties` file.

- `-report` generates a report after downloading test results. It is not effective in commands mode. It can be used with [auto mode]({{< relref "test-execution#auto-mode" >}}) using the `-auto` option to automatically generate a report once tests are downloaded.
