...
bc(java). public static List<HtmlElement> waitForHtmlElements(HtmlPage
...
bq(note).. Note that when creating AWS instances from AMIs you will see
...
bq(warning).. When using the new EC2 management UI for XLT, please be
...
![](/images/releasenotes/xlt333/network-statistics-small.jpg):/images/releasenotes/xlt333/network-statistics-large.png
<span class="caption">The new network statistics section</span>
...
![](/images/releasenotes/xlt331/xlt-comments.png) <span
class="caption">Some example text demonstrating the comment
feature</span>
...
com.xceptance.xlt.loadtests.comment.9 = Here comes another one,
completely out of order, but valid.
...
![](/images/releasenotes/xlt33/comparison-report-actions-small.jpg):/images/releasenotes/xlt33/comparison-report-actions-large.png
<span class="caption">An example for comparing action performance</span>
...
`create\_diff\_report.sh <baseline_report_dir>`
...
`create\_trend\_report.sh <report_dir_1> <report_dir_2> …`
...
![](/images/releasenotes/xlt33/trend-report-chart-small.png):/images/releasenotes/xlt33/trend-report-chart-large.png
<span class="caption">A trend chart</span>

Additionally, the run time difference values (in percent) are displayed
in two tables with color-coded cells, as seeen in the comparison
...
![](/images/releasenotes/xlt33/trend-report-requests-small.png):/images/releasenotes/xlt33/trend-report-requests-large.png
<span class="caption">The run time differences relative to the first
test run</span>
...
`../../bin/jruby.sh`
com.xceptance.xlt.samples.tests.jruby.TVisitor  
...
\# get the HTML form  
form = driver.find\_element\_by\_id(“sampleForm”)

\# get the input elements  
input\_list = form.find\_elements\_by\_name(“name”)  
Assert.assert\_equals(2, input\_list.size())

\# fill in the form  
input\_list.at(0).send\_keys(“Bob”)  
input\_list.at(1).send\_keys(“Marley”)
...
bq(note).. If you don’t specify one of the property values mentioned
...
`\#\# The default time (in ms) to wait for JavaScript background`
jobs to finish.
