...
Center](https://lab.xceptance.de/versions/88).
...
changes and view the current roadmap in the [XLT Information
Center](https://lab.xceptance.de/versions/86).
...
changes and view the current roadmap in the [XLT Information
Center](https://lab.xceptance.de/versions/85).
...
changes and view the current roadmap in the [XLT Information
Center](https://lab.xceptance.de/versions/84).
...
changes and view the current roadmap in the [XLT Information
Center](https://lab.xceptance.de/versions/81).
...
changes and view the current roadmap in the [XLT Information
Center](https://lab.xceptance.de/versions/68).
...
-   In the script tree view on the left-hand side, use the new keyboard
    shortcut \<kbd\>Ctrl\</kbd\>+\<kbd\>A\</kbd\> to select all
...
`\#\# scale the load down to 50% for TAuthor and to 10% for all`
other scenarios  
...
1.  number of result browsers per agent and type of error  
    com.xceptance.xlt.output2disk.onError.limiter.maxDumps = 10

<!-- -->

1.  period after which the result dump counter is reset to 0  
...
`\> agentcontroller.sh`
-Dcom.xceptance.xlt.agentcontroller.port=8501
...
bc(java).. ExclusiveDataProvider<String> dataProvider =
ExclusiveDataProvider.getInstance(“accounts.txt”);  
...
bc(java). ExclusiveDataProvider<Account> dataProvider =
ExclusiveDataProvider.getInstance(“accounts.txt”, new
AccountParser());  
...
1.  specific setting for the TAuthor transaction type  
    TAuthor.com.xceptance.xlt.maximumTransactionRunTime = 20000

<!-- -->

1.  specific setting for the TVisitor test case class  
    com.xceptance.xlt.samples.tests.TVisitor.com.xceptance.xlt.maximumTransactionRunTime
    = 3500
...
`\# use “firefox\_clientperformance” instead of “firefox”  `
xlt.webDriver = firefox\_clientperformance
...
that executes the requests one after another. The proxy recorder ships
