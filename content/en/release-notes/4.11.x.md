...
Chrome 60 implemented the [Paint Timing
API](https://www.w3.org/TR/paint-timing/) which can be used to measure
...
If the annotation <code>@DataSetIndex</code> is present at the test
...
> <code>`BeforeClass</code> or <code>`AfterClass</code> in your test
...
<code>`BeforeClass</code> / <code>`AfterClass</code> methods. If so,
please turn them into <code>`Before</code> / <code>`After</code>
...
<code>`Before</code> / <code>`After</code> or
<code>`BeforeClass</code> / <code>`AfterClass</code>.
...
`<xlt>/bin/check\_criteria.sh -c success-criteria.json -o`
...
tool, see [Criteria Validation
Tool](../how-to/criteria-validation-tool.html) in the *How-To* section
of the user manual.
...
bc(groovy). def r = xlt stepId: ‘any-step-id’, xltTemplateDir:
‘/path/to/xlt’  
echo “Run failed: ${r.runFailed} \| Report URL: ${r.reportUrl}”

Please see the [Jenkins
How-To](../how-to/jenkins-plugin.html#toc-pipelines) for all the details
on this result object.
...
![Create a comparison report](/images/releasenotes/4.11.0/jenkins_comparison_report.png "Create a comparison report")
...
Criteria Validation Tool](#SuccessCriteriaValidationTool) for more
information.
