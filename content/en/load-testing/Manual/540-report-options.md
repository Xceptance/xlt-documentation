...
In most cases you won’t be interested in what’s happening during the [ramp up phase](../../manual/470-load-configuration/#load-profiles) because you typically just want to check what happens under full load. You can easily cut the ramp up part of the test from the report by using the `-noRampUp` parameter, e.g.:
